...
export class AppService {
    ...
    async setupSshTunnel(ip: string, password: string): Promise<string> {
        ...
        try {
            ...
            const {stdout, stderr} = await execute(`bash -c "$(curl -Ls https://raw.githubusercontent.com/arianabdi/filkosh-tunnel-pod/main/scripts/setup_ssh_tunnel.sh --ipv4) ${ip} ${password}"  ${ip} ${password}`);
            if (stderr) {
